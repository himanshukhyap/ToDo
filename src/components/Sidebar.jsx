...
import {
  ...
  Plus, Sun, Moon, LogOut, Tag, Settings, Folder,
  MoreHorizontal, Pencil, Trash2, Check, X, Menu, PanelLeftClose,
} from "lucide-react";
...
/* ── Item row dots menu ───────────────────────────────── */
function ItemMenu({ onEdit, onDelete, label }) {
  ...
  return (
    <div className="sb-item-menu" ref={ref} onClick={e => e.stopPropagation()}>
      <button className="sb-item-menu-btn" onClick={() => setOpen(!open)}><MoreHorizontal size={13}/></button>
      {open && (
        <div className="sb-item-dropdown">
          <button className="sb-item-dd-btn" onClick={() => { onEdit(); setOpen(false); }}><Pencil size={12}/> Rename</button>
          <button className="sb-item-dd-btn danger" onClick={() => { onDelete(); setOpen(false); }}><Trash2 size={12}/> Delete</button>
        </div>
      )}
    </div>
  );
...
function SbGroup({ label, icon, children, defaultOpen = true, badge }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="sb-group">
      <button className="sb-group-header" onClick={() => setOpen(!open)}>
...
/* ── Sidebar ──────────────────────────────────────────── */
export default function Sidebar({ active, setActive, activeCat, setActiveCat, activeNotebook, setActiveNotebook, collapsed, setCollapsed }) {
  const { user, logout }        = useAuth();
  const { theme, toggle }       = useTheme();
  const { categories, addCategory, updateCategory, deleteCategory } = useCategories();
  const { notebooks, add: addNb, update: updateNb, remove: removeNb } = useNotebooks();
  ...
  if (collapsed) return (
    <aside className="sidebar sidebar-collapsed">
      ...
      <div className="sb-collapsed-bottom">
        <button className="sb-col-icon" onClick={toggle} title="Toggle theme">{theme==="dark"?<Sun size={17}/>:<Moon size={17}/>}</button>
        <div className="avatar-fallback sm" onClick={() => setCollapsed(false)}>{initials}</div>
      </div>
    </aside>
  );
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sb-brand">
        ...
        <button className="sb-collapse-btn ml-auto" onClick={() => setCollapsed(true)} title="Collapse sidebar">
      ...
      <nav className="sb-nav">
        ...
        {/* ── TASKS ───────────────────────────── */}
        <SbGroup label="Tasks" icon={<CheckSquare size={15}/>} badge={0}>
          ...
          {addingCat ? (
            <div className="sb-add-form">
              ...
              <div className="sb-add-actions">
                ...
                <button className="sb-add-ok" onClick={()=>{ if(newCatName.trim()){ addCategory(newCatName,newCatColor); setNewCatName(""); setAddingCat(false); } }}>
                  <Check size={12}/>
                </button>
              </div>
            </div>
        ...
        <SbGroup label="Notes" icon={<StickyNote size={15}/>}>
          <button className={`sb-item ${active==="notes" ? "active" : ""}`}
          ...
          {notebooks.map(nb => (
            editNbId === nb.id ? (
              <div key={nb.id} className="sb-item">
                ...
                <InlineEdit value={nb.name}
                  onSave={v => { updateNb(nb.id, v, nb.color); setEditNbId(null); }}
              ...
              <button key={nb.id}
                ...
                <span className="sb-item-dot" style={{background:nb.color}}/>
                <span className="sb-item-label">{nb.name}</span>
                <ItemMenu
                  label={nb.name}
                  onEdit={() => setEditNbId(nb.id)}
                  onDelete={async () => {
                    const ok = await confirmDelete(`notebook "${nb.name}"`);
                    if (!ok) return;
                    try {
                      await removeNb(nb.id);
                      if (activeNotebook?.id === nb.id) setActiveNotebook(null);
                    } catch (e) {
                      errorAlert(`Could not delete notebook: ${e.message}`);
                    }
                  }}
                />
              </button>
          ...
          {addingNb ? (
            <div className="sb-add-form">
              <input className="sb-add-input" placeholder="Notebook name…" value={newNbName} autoFocus
                ...
                onKeyDown={e => {
                  if(e.key==="Enter" && newNbName.trim()){ addNb(newNbName,newNbColor); setNewNbName(""); setAddingNb(false); }
              ...
              <div className="sb-add-actions">
                ...
                <button className="sb-add-ok" onClick={()=>{ if(newNbName.trim()){ addNb(newNbName,newNbColor); setNewNbName(""); setAddingNb(false); } }}>
                  <Check size={12}/>
                </button>
              </div>
            </div>
      ...
      <div className="sb-footer">
        <button className="sb-footer-btn" onClick={toggle} title={theme==="dark"?"Light mode":"Dark mode"}>
          {theme==="dark" ? <Sun size={16}/> : <Moon size={16}/>}
          <span>{theme==="dark" ? "Light mode" : "Dark mode"}</span>
        </button>

        <div className="sb-user" ref={userRef}>
          <button className="sb-user-trigger" onClick={() => setUserOpen(!userOpen)}>
            {user?.photoURL
