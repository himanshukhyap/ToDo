...
import { Shield, Clock3, Users, Mail, CheckCircle2, Save, Download, Upload } from "lucide-react";
import { useAuth } from "../context/AuthContext";
import { useTasks } from "../hooks/useTasks";
import { useNotes } from "../hooks/useNotes";
import { useNotebooks } from "../hooks/useNotebook";
import { errorAlert, toast } from "../utils/swal";
import { exportUserBackup, importUserBackup } from "../services/backupService";
...
export default function AdminPanel() {
  ...
  const handleImportFile = async (event) => {
    ...
    try {
      ...
      const summary = await importUserBackup(user, text);
      toast("success", `Import complete: ${summary.tasks} tasks, ${summary.notes} notes, ${summary.notebooks} notebooks`);
    } catch (e) {
    ...
    <section className="tab-panel admin-panel">
      <div className="admin-hero">
        <div>
          ...
          <h2>Workspace controls and session policy</h2>
          <p>
            Signed in as <strong>{user?.email}</strong>. Admin access is granted by email allowlist.
          </p>
        </div>
      ...
      <div className="admin-stats">
        ...
        <article className="admin-stat-card">
          ...
          <strong>{tasks.length + notes.length + notebooks.length} items</strong>
        ...
        <article className="admin-card">
          ...
          <div className="admin-settings-form">
            <p className="admin-help-text">
              JSON backup download karein aur isi panel se merge-import bhi karein. Import existing data ko delete nahi karta.
            </p>
            ...
            <ul className="admin-policy-list">
              <li>Backup me tasks, notes, categories, notebooks, sections, pages aur trash include hote hain.</li>
              <li>Import current account me data add karta hai, existing records ko remove ya replace nahi karta.</li>
              <li>Sab imported records current logged-in admin user ke account ke under save honge.</li>
            </ul>
          </div>
        </article>
